> **Nota de Contexto:**
> Este documento provee los detalles técnicos de bajo nivel de las mejoras de localización (i18n) implementadas en adJ. Para una visión general de cómo estas mejoras se integran en el proceso de construcción de la distribución, por favor consulte el [PROCESO-DE-CONSTRUCCION.md](../PROCESO-DE-CONSTRUCCION.md).
...
- **Hilos (Threading):** La implementación de `xlocale` en `adJ` **no es segura para hilos** (*thread-safe*) si se intenta usar un *locale* diferente en cada hilo. Debido a que el sistema aún no implementa un *Thread Control Block* (TCB) y utiliza variables globales para el estado del *locale*, un cambio en un hilo afectará a todos los demás hilos del proceso de forma impredecible.
- **Señales e Interrupciones:** No se garantiza un comportamiento consistente en aplicaciones que utilicen manejadores de señales o interrupciones que cambien el *locale*. Si un manejador de señal cambia el *locale* global, este cambio persistirá cuando el control vuelva al flujo principal del programa, lo que puede causar corrupción de datos o formatos inesperados.
- **Uso Recomendado:** Se recomienda establecer el *locale* deseado al inicio de la aplicación y evitar cambios dinámicos en entornos concurrentes o reentrantes.

Compatibilidad Binaria:
-----------------------

- **OpenBSD:** `adJ` no garantiza compatibilidad binaria total con OpenBSD estándar. 
- **Renombre de Símbolos:** En particular, los programas que dependen de la función `daemon()` o de constantes y símbolos que contengan la palabra `daemon` podrían presentar problemas, ya que en el sistema base de `adJ` se ha realizado un renombre sistemático de `daemon` por `servicio` (ej. `servicio()`, `LOG_SERVICIO`, etc.). Se han incluido envoltorios de compatibilidad, pero el uso de los nuevos nombres es preferido y esperado por las herramientas del sistema.
